...
/// <summary>
/// A TextBox control that shows a suggestion "inside itself".
...
public sealed class AutoCompleteTextBox : TextBox
{
    ...
    private string GetSuggestion(out int incompleteLength)
    {
        if (Text.Length > 0 && SuggestionsSource is not null)
        {
            var lastSpaceIndex = Text.LastIndexOf(' ');
            var incompleteWord = Text.Substring(lastSpaceIndex + 1);
            incompleteLength = incompleteWord.Length;
            if (string.IsNullOrWhiteSpace(incompleteWord))
                return string.Empty;
    ...
    private void AcceptSuggestion()
    {
        ...
        string suggestion = GetSuggestion(out var incompleteLength);

        if (suggestion.Length > 0)
        {
            Text += suggestion.Substring(incompleteLength);
    ...
    private void ShowSuggestion()
    {
        ...
        {
            CurrentSuggestion = GetSuggestion(out var incompleteLength);

            if (CurrentSuggestion.Length > 0)
            {
                SuggestionControl.Content = $"{SuggestionPrefix}{CurrentSuggestion[incompleteLength..]}{SuggestionSuffix}";
